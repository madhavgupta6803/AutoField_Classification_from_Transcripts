...
def get_doc_labels(doc_true, doc_pred):
    ...
    with open(doc_true, "r", newline='', encoding='utf-8') as file_true:
        true_labels = [row[-1] for row in csv.reader(file_true, delimiter='\t')]
        if 'field' in true_labels:
            true_labels.remove('field')

    with open(doc_pred, "r", newline='', encoding='utf-8') as file_pred:
        pred_labels = [row[-1] for row in csv.reader(file_pred, delimiter='\t')]
        if 'field' in pred_labels:
            pred_labels.remove('field')
...
def get_dataset_labels(true_path, pred_path, save=False):
    ...
    print(df)
    if save == True:
        df.to_csv("eval_metrics_Random_Forest_20.tsv")
...
if __name__ == "__main__":
    ...
    doc_true = "dataset/val_w_ann/boxes_transcripts_labels"#f"{os.getcwd()}/
    doc_pred = "dataset/output"#f"{os.getcwd()}
